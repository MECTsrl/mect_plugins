...
{
  hour=tableWidgetHours->item(row,col)->text().toInt()*3600;
  te->setTime(QTime().addSecs(hour+min+sec));
}
...
{
  for(int tabNumber=0;tabNumber<tabMinWidget->count();tabNumber++)
    {
      if(tabMinWidget->currentIndex() == tabNumber)
        {
          min=tableWidgetMin[tabNumber]->item(row,col)->text().toInt()*60;
          te->setTime(QTime().addSecs(hour+min+sec));
        }
    }
...
{
  for(int tabNumber=0;tabNumber<tabSecWidget->count();tabNumber++)
    {
      if(tabSecWidget->currentIndex() == tabNumber)
        {
          sec=tableWidgetSec[tabNumber]->item(row,col)->text().toInt();
          te->setTime(QTime().addSecs(hour+min+sec));
        }
    }
}
