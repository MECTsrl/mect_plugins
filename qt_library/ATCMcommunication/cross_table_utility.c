...
size_t fillSyncroArea(void)
{
	...
	while (fgets(line, LINE_SIZE, fp) != NULL)
	{
		...
		{
			sprintf(CrossTableErrorMsg, "Tag '%s' too long, maximum lenght is %d", token, TAG_LEN);
			LOG_PRINT(error_e, "%s at line %d.\n", CrossTableErrorMsg, elem_nb);
			return elem_nb;
		}
		...
		{
			varNameArray[elem_nb].type = uint_e;
		}
		else if (strcmp(token, "INT") == 0)
		{
			varNameArray[elem_nb].type = int_e;
		}
		else if (strcmp(token, "UDINTABCD") == 0)
		{
		...
		else if (strcmp(token, "DINTABCD") == 0)
		{
		...
		if (elem_nb > 1)
		{
			...
			if (varNameArray[elem_nb].block != 0 && varNameArray[elem_nb].block == varNameArray[elem_nb - 1].block)
			{
				...
				if (varNameArray[elem_nb].block != 0 && elem_nb > 0 && varNameArray[elem_nb].type != varNameArray[elem_nb - 1].type)
				{
					...
					LOG_PRINT(error_e, "%s at line %d. %d -> %d, bitperblock %d\n", CrossTableErrorMsg, elem_nb, varNameArray[elem_nb].type, varNameArray[elem_nb - 1].type, bitperblock);
					return elem_nb;
				}
				...
				if (varNameArray[elem_nb].node != INTERNAL_VARIABLE_FAKE_NODEID && ((ActualAddress != LastAddress + 1 && varNameArray[elem_nb - 1].type <= int_e) || (ActualAddress != LastAddress + 2 && varNameArray[elem_nb - 1].type > int_e)) && (bitperblock == 0 || bitperblock >= 8))
				{
					sprintf(CrossTableErrorMsg, "Malformed 'Block', not consecutive address");
					LOG_PRINT(error_e, "%s at line %d. %d -> %d, bitperblock %d type %d\n", CrossTableErrorMsg, elem_nb, LastAddress, ActualAddress, bitperblock, varNameArray[elem_nb - 1].type);
					return elem_nb;
				}
	...
	for(i = 1; i < elem_nb; i++)
	{
		...
		if (setupConnectedDevice(_protocol_map_[varNameArray[i].protocol], varNameArray[i].node) == 0)
		{
...
#if 0
	/* untill there are elements with the same block */
	for (j = *CtIndex; j < DB_SIZE_ELEM && cmpCrossTableBlock(*CtIndex, j) == 0; j++)
	{
		LOG_PRINT(info_e, "%s - Actual block %d\n", varNameArray[j].tag, varNameArray[j].block);
		/* check if the element is already into the syncro vector */
		for (i = 0; i < SyncroAreaSize; i++)
		{
			LOG_PRINT(verbose_e, "pIOSyncroAreaO[%d] '%X'\n", i, pIOSyncroAreaO[i]);
			/* element already preset */
			if (cmpSyncroCtIndex(pIOSyncroAreaO[i], j) == 0)
			{
				found = 1;
				break;
			}
		...
		/* if the element is not already present, add it */
		if (found == 0)
		{
			ret += addSyncroElementbyIndex(varNameArray[j].tag, j);
		}
	}
	return ret;
#else
...
int readFromDb(int ctIndex, void * value)
{
    ...
    /* fieldbus in error */
    switch (varNameArray[ctIndex].protocol)
    {
    case prot_rtu_e:
        if (getErrorBit(TAG_RTU, TAG_BLACKLIST, varNameArray[ctIndex].node) == 1)
        {
            LOG_PRINT(info_e, "Communication error for channel '%s'\n", TAG_RTU);
            return -1;
        }
        break;
    case prot_tcp_e:
        if (getErrorBit(TAG_TCP, TAG_BLACKLIST, varNameArray[ctIndex].node) == 1)
        {
            LOG_PRINT(info_e, "Communication error for channel '%s'\n", TAG_TCP);
            return -1;
        }
        break;
    case prot_tcprtu_e:
        if (getErrorBit(TAG_TCPRTU, TAG_BLACKLIST, varNameArray[ctIndex].node) == 1)
        {
            LOG_PRINT(info_e, "Communication error for channel '%s'\n", TAG_TCPRTU);
            return -1;
        }
...
int writeToDb(int ctIndex, void * value)
{
    ...
    case prot_rtu_e:
        if (getErrorBit(TAG_RTU, TAG_BLACKLIST, varNameArray[ctIndex].node) == 1)
        {
            LOG_PRINT(error_e, "Communication error for channel '%s'\n", TAG_RTU);
            return -1;
        }
        break;
    case prot_tcp_e:
...
int isBlockActiveByCtIndex(const int CtIndex, char * varblockhead)
{
	...
	{
		LOG_PRINT(verbose_e, "Block %d is aready active var %s, block %s.\n", block, varNameArray[CtIndex].tag, varNameArray[CtIndexHead].tag);
		return 1;
	}
	else
	{
		LOG_PRINT(verbose_e, "Block %d is not active, activate it.\n", block);
		return 0;
	}
}
...
int isDeviceConnected(const char * protocol, int node)
{
	...
	if (strcmp(TAG_RTU, protocol) != 0 && strcmp(TAG_TCP, protocol) != 0 && strcmp(TAG_TCPRTU, protocol) != 0 )
	{
		LOG_PRINT(error_e, "Invalid protocol %s\n",protocol);
		return -1;
	}
	if(strcmp(TAG_RTU, protocol) == 0)
	{
		LOG_PRINT(error_e, "RTU %d %d\n",node, device_status[prot_rtu_e][node]);
		return device_status[prot_rtu_e][node];
	}
	else if(strcmp(TAG_TCP, protocol) == 0)
	{
		LOG_PRINT(error_e, "TCP %d %d\n",node, device_status[prot_tcp_e][node]);
		return device_status[prot_tcp_e][node];
	}
	else if(strcmp(TAG_TCPRTU, protocol) == 0)
	{
		LOG_PRINT(error_e, "TCPRTU %d %d\n",node, device_status[prot_tcprtu_e][node]);
		return device_status[prot_tcprtu_e][node];
	}
	LOG_PRINT(error_e, "Invalid protocol %s\n",protocol);
	return -1;
}
...
int setupConnectedDevice(const char * protocol, int node)
{
	...
	if(strcmp(TAG_RTU, protocol) == 0)
	{
		device_status[prot_rtu_e][node] = 1;
		return 0;
	}
	else if(strcmp(TAG_TCP, protocol) == 0)
	{
		device_status[prot_tcp_e][node] = 1;
		return 0;
	}
	else if(strcmp(TAG_TCPRTU, protocol) == 0)
	{
		device_status[prot_tcprtu_e][node] = 1;
		return 0;
	}
	return 1;	
}
...
int disconnectDeviceByVarname(const char * varname)
{
	...
	if( varNameArray[CtIndex].node != 0 && varNameArray[CtIndex].node != INTERNAL_VARIABLE_FAKE_NODEID && varNameArray[CtIndex].protocol != prot_none_e)
	{
		device_status[varNameArray[CtIndex].protocol][varNameArray[CtIndex].node] = 0;
...
int connectDeviceByVarname(const char * varname)
{
	...
	if ( varNameArray[CtIndex].node != 0 && varNameArray[CtIndex].node != INTERNAL_VARIABLE_FAKE_NODEID  && varNameArray[CtIndex].protocol != prot_none_e)
	{
		device_status[varNameArray[CtIndex].protocol][varNameArray[CtIndex].node] = 1;

		/*Add activation of eventually disconnected variables beloging to P, S categories*/
		int i;
		int node = varNameArray[CtIndex].node;
...
int isDeviceConnectedByVarname(const char * varname)
{
	...
	if (Tag2CtIndex(varname, &CtIndex) != 0)
	{
		LOG_PRINT(info_e, "'%s' not found into DataVector - Ctindex %d\n", varname, CtIndex);
		return -1;
	}
	if (varNameArray[CtIndex].node == 0 || (varNameArray[CtIndex].node == INTERNAL_VARIABLE_FAKE_NODEID && varNameArray[CtIndex].protocol == prot_none_e))
	{
	...
	{
		return device_status[varNameArray[CtIndex].protocol][varNameArray[CtIndex].node];
	}
}
...
int resetError(const char *protocol)
{
	...
	{	 if(strcmp(protocol, TAG_TCP) == 0)
		...
		{
			if(strcmp(protocol, TAG_TCPRTU) == 0)
			{
...
{
	short int value;
	if (strcmp(kind, TAG_BLACKLIST) != 0 && strcmp(kind, TAG_OTHER) != 0)
	{
		LOG_PRINT(error_e, "Requested error kind does not exist. Allowed are '%s - %s'\n", TAG_BLACKLIST, TAG_OTHER);
		return -1;
	}
	if(strcmp(protocol, TAG_RTU) == 0)
	{
...
{
	int value;
	if ((strcmp(kind, TAG_BLACKLIST) != 0 && strcmp(kind, TAG_OTHER) != 0) || (node <= 0 || node > MAX_DEVICE_NB) )
	{
		LOG_PRINT(error_e, "Requested error bit does not exist. Allowed error bit are '%s - %s' for nodes %d (%d-%d)\n",
				TAG_BLACKLIST, TAG_OTHER, node, 1, MAX_DEVICE_NB);
		return -1;
	}
