...
cpp_leading_comments_write(FILE *file)
{
    ...
    fprintf(file, "/* File automatically generated by the Crosstable compiler */\n");
    fprintf(file, "/* from the Crosstable file %s. */\n", crosstable.filename);
...
usage(char *pn)
{
    ...
    fprintf(stderr, "  -c <file name>   - input Crosstable file name (CSV format)\n");
...
main(int argc, char *argv[])
{
    ...
    if (crosstable.filename == NULL)
        error_log_exit(0, "no Crosstable file name.");

    FILE *fp = fopen(crosstable.filename, "rb");
    if (fp == NULL)
        perror_log_exit(0, "cannot open Crosstable file.");
    ...
    while ((bytes_read = fread(buf, 1, FIELD_MAX_LENGTH, fp)) > 0)
        if (csv_parse(&p, buf, bytes_read, cb_field, cb_row, &row) != bytes_read)
            cperror_log_exit(&p, crosstable.row_crt + 1, "while parsing the Crosstable");

    csv_fini(&p, cb_field, cb_row, &row);

    if (ferror(fp))
        perror_log_exit(crosstable.row_crt, "while reading Crosstable file");
